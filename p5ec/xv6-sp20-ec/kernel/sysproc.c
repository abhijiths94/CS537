...
int sys_sem_init(void)
{
    ...
    if(argptr(0, (void*) &sem_id, 4) < 0 || argint(1, &count) < 0)
        return -1;
    return proc_sem_init(sem_id, count);
}
...
int sys_sem_destroy(void)
{
    ...
    if(argint(0, &sem_id) < 0)
        return -1;
    return proc_sem_destroy(sem_id);
}
